...
class DesignSpaceBase(object):
    ...
    def getOutline(self, glyphName, location, penFactory):
        """Creates an outline for the requested glyph and location, using
        penFactory to construct a pen. It will be called with a glyphSet
        argument. Returns the path/pen, a centerPt and a size as a 3-tuple.
        """
        # XXX it needs to be seen whether the center + size thing actually
        # works.  Maybe a rectangle (not necessarily the bounding box) works
        # better.
        raise NotImplementedError()

    def getGlyphName(self, charCode):
        """Returns the glyph name associated with charCode (a Unicode code
        point). Should return '.notdef' if the character is not defined."""
        raise NotImplementedError()
