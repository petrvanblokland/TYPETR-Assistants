...
from utilities import padData, calcHeadCheckSumAdjustment, calcIntBounds
...
def _setTransformBits(flag, transform):
    if transform == 1:
        flag |= 1 << 7
    elif transform == 2:
        flag |= 1 << 7
...
def packTestMetadata(t, havePrivateData=False):
    origMetadata, compMetadata = t
    if havePrivateData:
