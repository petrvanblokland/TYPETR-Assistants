...
from tnbits.compilers.f5.unicodeRangesData import unicodeRanges, otScriptTags
...
def distributeUnicodes(unicodes):
    ...
    characters that don't fall into any range.
        ...
        {None: [100000]}
        >>> sorted(distributeUnicodes([65, 165]).items())
        [('Basic Latin', [65]), ('Latin-1 Supplement', [165])]
        >>> unicodes = list(range(65, 70)) + list(range(6000, 6005)) + [100000]
        >>> ranges = distributeUnicodes(unicodes)
        >>> sorted(ranges.items(), key=lambda x: ('', x[1]) if x[0] is None else x)
        [(None, [100000]), ('Basic Latin', [65, 66, 67, 68, 69]), ('Tagbanwa', [6000, 6001, 6002, 6003, 6004])]
        >>> all = set()
...
def _distributeUnicodes_ReferenceImplementation(unicodes):
    ...
    """
        ...
        ...
        >>> all.sort()
        >>> assert sorted(all) == sorted(unicodes)
    """
    ranges = {}
    for uni in unicodes:
...
def countCoverageByRangeName(unicodes):
    ...
    will be (count, None).
        ...
        {'Basic Latin': (2, 128)}
        >>> sorted(countCoverageByRangeName([65, 66, 600]).items())
...
def _countCoverage(unicodes, byName=False):
    """
        ...
        {'Basic Latin': (2, 128)}
        >>> sorted(_countCoverage([65, 66, 600], byName=True).items())
        [('Basic Latin', (2, 128)), ('IPA Extensions', (1, 96))]
...
def unpackRangeBits(ur1, ur2, ur3, ur4):
    ...
    from the OS/2 table, return a set of bit numbers.
        ...
        >>> sorted(unpackRangeBits(0x0, 0x0, 0x0, 0x0))
        []
        >>> sorted(unpackRangeBits(0x1, 0x0, 0x0, 0x0))
        [0]
        >>> sorted(unpackRangeBits(0x1, 0x1, 0x1, 0x1))
        [0, 32, 64, 96]
        >>> sorted(unpackRangeBits(0xffffffff, 0x1, 0x2, 0x4))
