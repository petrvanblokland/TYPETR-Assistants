...
from fontTools.ttLib.tables.otTables import ScriptRecord, Script, LangSysRecord, LangSys, FeatureRecord, Feature
#from fontTools.misc.py23 import str, range
from fontTools.ttLib.tables.otBase import BaseTable

# XXX A little hack to make Python 3 fontTools OTL objects be hashable again.
# Obviously needs a better solution.
def _hashHack(self):
    return id(self)
BaseTable.__hash__ = _hashHack
...
def mergeFeatures(table1, table2):
    ...
    # Merge the script lists.
    scriptRecords1 = _scriptListToDict(table1.ScriptList.ScriptRecord)
    scriptRecords2 = _scriptListToDict(table2.ScriptList.ScriptRecord)
    mergedScriptRecords = _mergeScriptLists(scriptRecords1, scriptRecords2)

    # find duplicate features
    featuresToBeMerged = {}
    for script in mergedScriptRecords.values():
        for langSys in script.values():
            if langSys is None:
                continue
            doMerge = _findDuplicateFeatureTypes(table1, langSys)
            for featureIndices in doMerge:
                assert len(featureIndices) == 2, featureIndices
                if featureIndices in featuresToBeMerged:
                    featuresToBeMerged[featureIndices].append(langSys)
                else:
                    featuresToBeMerged[featureIndices] = [langSys]

    newFeatureIndex = len(table1.FeatureList.FeatureRecord)
    for mainIndex, secondaryIndex in featuresToBeMerged:
        ...
        assert main.FeatureTag == secondary.FeatureTag
        lookupIndices = []
        lookupIndices.extend(main.Feature.LookupListIndex)
        for lookupIndex in secondary.Feature.LookupListIndex:
            if lookupIndex not in main.Feature.LookupListIndex:
                lookupIndices.append(lookupIndex)
        newFeatureRecord = FeatureRecord()
        newFeatureRecord.FeatureTag = main.FeatureTag
        newFeatureRecord.Feature = Feature()
        newFeatureRecord.Feature.LookupListIndex = lookupIndices
        newFeatureRecord.Feature.FeatureParams = main.Feature.FeatureParams or secondary.Feature.FeatureParams # XXX conflict?
        table1.FeatureList.FeatureRecord.append(newFeatureRecord)

        assert table1.FeatureList.FeatureRecord[newFeatureIndex] is newFeatureRecord

        for langSys in featuresToBeMerged[mainIndex, secondaryIndex]:
            if langSys.ReqFeatureIndex != 0xffff:
                if langSys.ReqFeatureIndex in (mainIndex, secondaryIndex):
                    # XXX This can't be quite right
                    langSys.ReqFeatureIndex = newFeatureIndex
            assert langSys.FeatureIndex.count(secondaryIndex) == 1
            langSys.FeatureIndex.remove(secondaryIndex)
            assert langSys.FeatureIndex.count(mainIndex) == 1
            i = langSys.FeatureIndex.index(mainIndex)
            assert i >= 0
            langSys.FeatureIndex[i] = newFeatureIndex
    ...
    for script in mergedScriptRecords.values():
        for langSys in script.values():
            if langSys is None:
                continue
            doMerge = _findDuplicateFeatureTypes(table1, langSys)
            assert not doMerge, doMerge

    table1.ScriptList.ScriptRecord = _scriptListFromDict(mergedScriptRecords)

    allUsedFeatureIndices = set()
    for scriptRecord in mergedScriptRecords.values():
        for langSys in scriptRecord.values():
            allUsedFeatureIndices.update(langSys.FeatureIndex)
            if langSys.ReqFeatureIndex != 0xffff:
                allUsedFeatureIndices.add(langSys.ReqFeatureIndex)

    featuresToDelete = set(range(len(table1.FeatureList.FeatureRecord))) - allUsedFeatureIndices
...
def calcMaxContext(font):
    """Attempt to calculate the OS/2 usMaxContext value."""
    maxContext = 1
    if "kern" in font:
        maxContext = 2
    for tag in ["GSUB", "GPOS"]:
        ...
        for lookup in table.LookupList.Lookup:
            for subTable in lookup.SubTable:
                if (tag == "GSUB" and subTable.LookupType == 7) or (tag == "GPOS" and subTable.LookupType == 9):
                    subTable = subTable.ExtSubTable
                if hasattr(subTable, "BacktrackGlyphCount"):
                    totalContext = subTable.BacktrackGlyphCount + subTable.InputGlyphCount + subTable.LookAheadGlyphCount
                    maxContext = max(maxContext, totalContext)
                elif tag == "GPOS" and subTable.LookupType in (2,):
                    maxContext = max(maxContext, 2)
...
class GlyphDeleter(LookupTraverser):
    ...
    def deleteGlyphs_SingleSubstFormat1(self, subTable, glyphNames):
        for input, output in list(subTable.mapping.items()):
    ...
    def deleteGlyphs_LigatureSubstFormat1(self, subTable, glyphNames):
        for initialGlyph, ligatures in list(subTable.ligatures.items()):
    ...
    def deleteGlyphs_AlternateSubstFormat1(self, subTable, glyphNames):
        for input, alternates in list(subTable.alternates.items()):
...
def _remapLookupsAndPruneFeatures(table, lookupRemap):
    ...
    if hasattr(table, "FeatureVariations"):
        raise NotImplementedError("Can't yet remap lookups in FeatureVariations")
    featuresToDelete = []
...
def _remapFeaturesAndPruneScripts(table, featureRemap):
    ...
    if hasattr(table, "FeatureVariations") and table.FeatureVariations is not None:
        # XXX This will fail if the "source" feature is supposed to be deleted:
        # for now, this only supports (re)sorting, not subsetting.
...
def _mergeLangSys(langSys1, langSys2, scriptTag, langSysTag):
    if langSys2 is None:
        return _copyLangSys(langSys1)
    if langSys1 is None:
        return _copyLangSys(langSys2)
    ...
    mergedLangSys = LangSys()
    mergedLangSys.LookupOrder = None

    if langSys1.ReqFeatureIndex == 0xffff:
        mergedLangSys.ReqFeatureIndex = langSys2.ReqFeatureIndex
    else:
        mergedLangSys.ReqFeatureIndex = langSys1.ReqFeatureIndex

    assert not set(langSys2.FeatureIndex) & set(langSys1.FeatureIndex)
    mergedLangSys.FeatureIndex = langSys1.FeatureIndex + langSys2.FeatureIndex  # the order here is arbitrary, we don't need to sort
    return mergedLangSys
...
def _findDuplicateFeatureTypes(table, langSys):
    ...
    for feaTag, feaIndices in features.items():
        if len(feaIndices) > 1:
            assert len(feaIndices) == 2, feaIndices
            featuresToBeMerged.add(tuple(sorted(feaIndices)))
...
def _validateDefaultScript(scriptRecord):
    assert len(scriptRecord) == 1
    assert "dflt" in scriptRecord


def _scriptRecordToDict(scriptRecord):
    d = {}
    for lsr in scriptRecord.Script.LangSysRecord:
        assert lsr.LangSysTag not in d
        d[str(lsr.LangSysTag)] = lsr.LangSys
    assert len(d) == len(scriptRecord.Script.LangSysRecord), (len(d), len(scriptRecord.Script.LangSysRecord))
    if scriptRecord.Script.DefaultLangSys is not None:
        d['dflt'] = scriptRecord.Script.DefaultLangSys
    return d

def _scriptRecordFromDict(scriptTag, script):
    scriptRecord = ScriptRecord()
    scriptRecord.ScriptTag = scriptTag
    scriptRecord.Script = Script()
    scriptRecord.Script.DefaultLangSys = script.get("dflt", None)
    langSysRecords = []
    for langSysTag, langSys in script.items():
        if langSysTag == 'dflt':
            continue
        langSysRecord = LangSysRecord()
        langSysRecord.LangSysTag = langSysTag
        langSysRecord.LangSys = langSys
        langSysRecords.append(langSysRecord)
    scriptRecord.Script.LangSysRecord = langSysRecords
    scriptRecord.Script.LangSysRecord.sort(key=lambda langSysRec: langSysRec.LangSysTag)
    for langSysRec in scriptRecord.Script.LangSysRecord:
        assert langSysRec.LangSysTag != 'dflt'
    return scriptRecord


def _scriptListToDict(scriptList):
    d = {}
    for scriptRecord in scriptList:
        assert scriptRecord.ScriptTag not in d
        d[str(scriptRecord.ScriptTag)] = _scriptRecordToDict(scriptRecord)
    assert len(d) == len(scriptList)
    return d

def _scriptListFromDict(scriptDict):
    scriptList = []
    ids = set()
    for scriptTag, script in scriptDict.items():
        assert id(script) not in ids
        ids.add(id(script))
        scriptRecord = _scriptRecordFromDict(scriptTag, script)
        scriptList.append(scriptRecord)
    scriptList.sort(key=lambda sr: sr.ScriptTag)
    return scriptList


# Inject a __repr__ method into LangSys, for easier debugging.
def _langSys__repr__(self):
    return "LangSys(%s, %s)" % (self.ReqFeatureIndex, self.FeatureIndex)
LangSys.__repr__ = _langSys__repr__


def _copyLangSys(langSys):
    if langSys is None:
        return None
    assert isinstance(langSys, LangSys)
    cp = LangSys()
    cp.ReqFeatureIndex = langSys.ReqFeatureIndex
    cp.LookupOrder = langSys.LookupOrder
    cp.FeatureIndex = list(langSys.FeatureIndex)
    return cp


def _mergeScriptLists(scriptRecords1, scriptRecords2):
    default1 = scriptRecords1.get('DFLT')
    default2 = scriptRecords2.get('DFLT')
    defaultLangSys1 = defaultLangSys2 = None
    if default1 is not None:
        _validateDefaultScript(default1)
        defaultLangSys1 = default1['dflt']
    if default2 is not None:
        _validateDefaultScript(default2)
        defaultLangSys2 = default2['dflt']

    #
    # Two steps:
    # 1. Propagate DFLT/dflt script/dflt to the others where needed, ensuring both sides
    # have the same scripts, and each script has the same languages.
    # 2. Merge.
    #

    # Step 1. Make both sides have the same scripts and languages by adding defaults

    scriptTags1 = set(scriptRecords1)
    scriptTags2 = set(scriptRecords2)

    for scriptTag in scriptTags1 - scriptTags2:
        script = {}
        for langSysTag in scriptRecords1[scriptTag]:
            script[langSysTag] = _copyLangSys(defaultLangSys2)
        scriptRecords2[scriptTag] = script

    for scriptTag in scriptTags2 - scriptTags1:
        script = {}
        for langSysTag in scriptRecords2[scriptTag]:
            script[langSysTag] = _copyLangSys(defaultLangSys1)
        scriptRecords1[scriptTag] = script

    for scriptTag in scriptTags1 & scriptTags2:
        s1 = scriptRecords1[scriptTag]
        s2 = scriptRecords2[scriptTag]
        localDefault1 = s1.get('dflt')   #, defaultLangSys1)
        localDefault2 = s2.get('dflt')   #, defaultLangSys2)
        langSysTags1 = set(s1)
        langSysTags2 = set(s2)
        for langSysTag in langSysTags1 - langSysTags2:
            s2[langSysTag] = _copyLangSys(localDefault2)
        for langSysTag in langSysTags2 - langSysTags1:
            s1[langSysTag] = _copyLangSys(localDefault1)

    # Step 2. Merge scriptRecords1 and scriptRecords2 into a new dict

    assert set(scriptRecords1) == set(scriptRecords2)
    mergedScriptRecords = {}
    for scriptTag in scriptRecords1:
        mergedScriptRecords[scriptTag] = {}
        s1 = scriptRecords1[scriptTag]
        s2 = scriptRecords2[scriptTag]
        assert set(s1) == set(s2)
        for langSysTag in s1:
            mergedScriptRecords[scriptTag][langSysTag] = _mergeLangSys(s1[langSysTag], s2[langSysTag], scriptTag, langSysTag)

    return mergedScriptRecords


# debug helpers

def _getFeatureTagsForLangSysDict(table, lsd):
    d = {}
    for k, v in lsd.items():
        tags = ["%s/%s" % (i, table.FeatureList.FeatureRecord[i].FeatureTag) for i in v.FeatureIndex]
        d[k] = " ".join(tags) + " " + str(v.ReqFeatureIndex)
    return d

def _getFeatureTags(table, scriptListDict):
    d = {}
    for k, v in scriptListDict.items():
        d[k] = _getFeatureTagsForLangSysDict(table, v)
    return d

def dumpScripts(table):
    from pprint import pprint
    d = _getFeatureTags(table, _scriptListToDict(table.ScriptList.ScriptRecord))
    pprint(d)
