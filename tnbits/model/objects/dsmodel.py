...
from tnbits.model.objects.glyph import getPoints, getComponents
...
class Axis(object):
    ...
    def __init__(self, tag, name=None, minimum=None, default=None, maximum=None, mapping=None, labelNames=None):
        ...
        ["<Axis XTRA min=AAA def=500 max=1000> Value not number ('AAA', 500, 1000)"]
    ...
    def __repr__(self):
        return '<%s %s min=%s def=%s max=%s>' % (self.__class__.__name__, self.tag, TX.asFormatted(self.minimum), TX.asFormatted(self.default), TX.asFormatted(self.maximum))
...
class BlendAxis(Axis):
    ...
    def __repr__(self):
        return '<%s %s min=%s def=%s max=%s masters=%d>' % (self.__class__.__name__, self.tag, TX.asFormatted(self.minimum), TX.asFormatted(self.default), TX.asFormatted(self.maximum), len(self.masters))
...
class DesignSpace(object):
    ...
    def _set_path(self, path):
        ...
        if path is not None:
            self._ds = ds = DesignSpaceDocument.fromfile(path)
    ...
    def _getCSV(self):
        ...
        for axis in self.axisList:
            ...
            lineMin.append(TX.asFormatted(axis.minimum))
            lineDef.append(TX.asFormatted(axis.default))
            lineMax.append(TX.asFormatted(axis.maximum))
        line.append('name')
        line.append('path')
        line.append('copy')
        ...
        for master in self.masterList:
            ...
            for axis in self.axisList:
                if axis.tag in master.location:
                    lineMaster.append(TX.asFormatted(master.location[axis.tag]))
                else:
                    lineMaster.append('')
