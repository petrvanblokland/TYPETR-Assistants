...
class TTF2WEB:
    ...
    def __init__(self,path, embed=None):
        if isinstance(path,TTFont):
            ...
            self.TTF = TTFont(path,recalcBBoxes=False)
        ...
        # See: https://www.microsoft.com/typography/otspec/os2.htm for possible values.
        if embed is not None:
            self.TTF['OS/2'].fsType = embed # 0 = allow embedding
    ...
    def getTTFCmap(self):
        for args in self.TTF_FIELDS['cmap']:
            cmap = self.TTF['cmap'].getcmap(*args)
    ...
    def getTTFOTFeatureTags(self):
        features = set()

        if 'GSUB' in self.TTF:

            for feat in self.TTF['GSUB'].table.FeatureList.FeatureRecord:
    ...
    def mungeNameFields(self,name="Unlicensed Font",url="http://webtype.com/"):
        ...
        for namerecord in self.TTF['name'].names:
            if namerecord.nameID in (1,4,8,9,16,3,6):
                #names to zero out to discourage desktop installation
                namerecord.string = uniqueid #some platforms don't like empty names
                #psname must match cff table font name or OS X font book will flag the font as invalid and it won't work in Safari 5.1
                if 'CFF ' in self.TTF and namerecord.nameID == 6:
                    self.TTF['CFF '].cff.fontNames[0] = namerecord.string
            elif namerecord.nameID in (0,7,10,13):
            ...
            if namerecord.platformID == 0 or (namerecord.platformID == 3 and namerecord.platEncID in (0, 1)):
                namerecord.string = namerecord.string.encode('utf_16_be')
            else:
                namerecord.string = namerecord.string.encode('latin1')
...
#            for feat in [x for x in self.TTF['GSUB'].table.FeatureList.FeatureRecord if x.FeatureTag in ('liga','smcp','salt')]:
            for feat in self.TTF['GSUB'].table.FeatureList.FeatureRecord:
                for lindex in feat.Feature.LookupListIndex:
                    lookup = self.TTF['GSUB'].table.LookupList.Lookup[lindex]
        ...
        for glyphname in self.TTF.getGlyphOrder():
            ...
            if glyphname not in glyphs2keep:
                ...
                if 'glyf' in self.TTF and glyphname in self.TTF['glyf']:
                    self.TTF['glyf'][glyphname] = Glyph()
                elif 'CFF ' in self.TTF and glyphname in cffstrings:
...
#            for feat in [x for x in self.TTF['GSUB'].table.FeatureList.FeatureRecord if x.FeatureTag in ('liga','smcp','salt')]:
    ...
    def createEOT(self,dest,compress=False):
        ...
        family = self.getTTFNameField('familyname')
        style = self.getTTFNameField('stylename')
        fullname = self.getTTFNameField('fullname')
        version = self.getTTFNameField('version')
        urls = []
    ...
    def createWOFF(self,dest,meta=""):
        ...
        tabletags = [k for k in self.TTF.keys() if len(k)==4 and k != 'DSIG']
        numtables = len(tabletags)

        flavor = 'CFF ' in tabletags and 0x4F54544F or 0x00010000
        ...
        for tag in tabletags: #original font order
            ...
            data = self.TTF.getTableData(tag)
    ...
    def createSVG(self, dest):
        ...
        svg.write("""<?xml version="1.0" standalone="yes"?>""")
        ...
        for uni,glyphname in cmap.items():
            ...
            if glyph['unicode'] in '"&<>':
                glyph['unicode'] = "&#x{0:02X};".format(uni)
    ...
    def dieargs(cls):
        print("""
Usage: python2.6 ttf2web.py [--chars=ABC] [--cprange1=number] [--no-subset]
                            [--font-weight=100|200|300|400|500|600|700|800|900|normal|bold]
                            [--test] [--gzip] [--metafile=file.xml]
                            [--txtfile=file.txt] [--no-final]
                            infiles outdir
Options:
 [SUBSETTING: if no subsetting options are specified, Latin 1 is the default]
 ...
 --no-subset: don't do any subsetting
 ...
 --no-final: don't include the WT "-final" bit in filenames
""")
        sys.exit(1)

if __name__ == "__main__":
    try:
        options, args = getopt.getopt(sys.argv[1:],'',['metafile=','txtfile=','cprange1=','chars=','font-weight=','test','gzip','no-subset','no-final'])
        ...
        if 'no-subset' not in options:
            if not options.get('cprange1'):
                options['cprange1'] = '1'
        ...
        fontfiles = set()
        for d in indir:
            for f in TTF2WEB.findFontsInDir(d,ext='ttf') + TTF2WEB.findFontsInDir(d,ext='otf'):
                fontfiles.add(f)

        for fontfile in fontfiles:
            outbase = outdir + '/' + os.path.basename(fontfile)[:-4]

            if 'no-final' in options:
                outfinal = outbase
            else:
                outfinal = outbase + '-final'

            munger = TTF2WEB(fontfile)
            ...
            if 'test' in options:
                css.write(munger.buildCSS(fontname=fontname,outpath=os.path.basename(outfinal)))
                teststring = u"AVALT oft fjord iffy find ĄąČčĘęĖė на русском языке Λάτρης της Ελλάδας ο διάσημος Βρετανός"
                html.write("""
                    <dt>All Formats CSS</dt>
                    <dd style='font-family:{fontname},"Comic Sans MS";'>{teststring}</dd>
                    <dt>TTF</dt>
                    <dd style='font-family:"{fontname} TTF","Comic Sans MS";'>{teststring}</dd>
                    <dt>EOT</dt>
                    <dd style='font-family:"{fontname} EOT","Comic Sans MS";'>{teststring}</dd>
                    <dt>WOFF</dt>
                    <dd style='font-family:"{fontname} WOFF","Comic Sans MS";'>{teststring}</dd>
                    <dt>SVG</dt>
                    <dd style='font-family:"{fontname} SVG","Comic Sans MS";'>{teststring}</dd>
                """.format(teststring=teststring,fontname=fontname).encode('utf-8'))

            if 'no-subset' not in options and options['chars']:
                munger.subsetGlyphs(options['chars'])

            #munger.mungeGlyphNames()
            munger.mungeNameFields()

            munger.generateSignature()

            ttffile = outfinal + ".ttf"
            munger.createTTF(ttffile)

            wofffile = outfinal + ".woff"
            munger.createWOFF(wofffile,meta=metainfo)

            eotfile = outfinal + ".eot"
            munger.createEOT(eotfile)
            munger.createEOT(outfinal + '-compressed.eot',compress=True)

            svgfile = outfinal + ".svg"
            ...
            if 'gzip' in options:
                for f in (ttffile,eotfile,svgfile):
                    gzfile = open(f + ".gz","wb")
                    ffile = open(f,"rb")
                    gz = gzip.GzipFile(filename="{0}-{1}".format(os.path.basename(outbase),flavors[f]),mode="wb",fileobj=gzfile)
                    gz.write(ffile.read())
                    gz.close()
                    ffile.close()
                    gzfile.close()
