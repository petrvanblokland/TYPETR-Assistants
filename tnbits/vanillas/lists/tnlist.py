...
class TnBitsTableViewSubclass(NSTableView):

    def mouseDown_(self, event):
        super(TnBbitsTableViewSubclass, self).mouseDown_(event)

    def keyDown_(self, event):
        didSomething = self.vanillaWrapper()._keyDown(event)
        if not didSomething:
            super(TnBitsTableViewSubclass, self).keyDown_(event)
    ...
    def textDidEndEditing_(self, notification):
        ...
        if info["NSTextMovement"] in [NSReturnTextMovement, NSTabTextMovement, NSBacktabTextMovement]:
            ...
            newNotification = NSNotification.notificationWithName_object_userInfo_(
                    ...
                    notification.object(),
                    info)
            super(TnBitsTableViewSubclass, self).textDidEndEditing_(newNotification)
            self.window().makeFirstResponder_(self)
        else:
            super(TnBitsTableViewSubclass, self).textDidEndEditing_(notification)

class _TnBbitsTableViewSubclass(TnBitsTableViewSubclass):

    def init(self):
        from warnings import warn
        warn(DeprecationWarning("_TnBitsTableViewSubclass is deprecated. Use TnBitsTableViewSubclass"))
        return super(_TnBitsTableViewSubclass, self).init()


class TnBitsArrayControllerObserver(NSObject):
...
class _TnBitsArrayControllerObserver(TnBitsArrayControllerObserver):

    def init(self):
        from warnings import warn
        warn(DeprecationWarning("_TnBitsArrayControllerObserver is deprecated. Use TnBitsArrayControllerObserver"))
        return super(_TnBitsArrayControllerObserver, self).init()


class TnBitsArrayController(NSArrayController):
...
class _TnBitsArrayController(TnBitsArrayController):

    def init(self):
        from warnings import warn
        warn(DeprecationWarning("_TnBitsArrayController is deprecated. Use TnBitsArrayController"))
        return super(_TnBitsArrayController, self).init()
...
class List(VanillaBaseObject):
    ...
    **otherApplicationDropSettings** A dictionary defining the drop settings when the source of the drop
    ...
    nsTableViewClass = TnBitsTableViewSubclass
    nsArrayControllerClass = TnBitsArrayController
    nsArrayControllerObserverClass = TnBitsArrayControllerObserver
