...
class StyleSheet(View):
    ...
    def getMenuItems(self):
        l =  [
                dict(title='Kerns', callback='kernsCallback:'),
                dict(title='Groups', callback='groupsCallback:'),
                dict(title='Explode', callback='explodeCallback:'),
    ...
    def kernsCallback_(self, menuItem):
        controller = self.getController()
        styleKeys = self.getSelectedKeys()

        for styleKey in styleKeys:
            controller.kerns(styleKey=styleKey)

    def groupsCallback_(self, menuItem):
        controller = self.getController()
        styleKeys = self.getSelectedKeys()

        for styleKey in styleKeys:
            controller.groups(styleKey=styleKey)

    def explodeCallback_(self, menuItem):
        controller = self.getController()
        styleKeys = self.getSelectedKeys()

        for styleKey in styleKeys:
            controller.explode(styleKey=styleKey)
