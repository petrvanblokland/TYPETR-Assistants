...
from tnbits.tools.constantsparts.smartsets import SS_ACCENTSCMB, ANCHOR_TOP
from tnbits.toolbox.transformer import TX
from tnbits.model.objects.glyph import getComponents, getPoints, isClockWise
...
from tnbits.analyzers.elements.stems import Stem, Bar, BlueBar, Counter, VerticalCounter, Width, DiagonalStem
...
class GlyphAnalyzer(PointAnalyzer):
    """The GlyphAnalyzer analyzes a DefCon glyph and finds key measures
    ...
    # See also SmartSets SS_ACCENTSCMD.
    #
    # from tnbits.constantparts.smartsets import SS_ACCENTSCMD
    #
    # TODO: Combine as import to make additions more consistent?
    DIACRITICS = set()
    TOPDIACRITICS = set()
    ...
    def resetGlyph(self, parent, name):
        """Initializes the GlyphAnalyzer from the no-overlap layer. We have to
        decide what happens if it exists but we don't know if it is up to date.

        NOTE: How slow would it be to always generate a new no-overlap?"""
        self.parent = parent
        self.name = name
        self.reset()
    ...
    def reset(self):
        ...
        self._horizontalCounters = None # Recognized straight counters
        self._horizontalRoundCounters = None
        self._horizontalMixedCounters = None
        ...
        self._HStem = None # Used as normalized reference in to other glyphs in the font.
        self._OStem = None
        ...
        self._blueBars = None # Bars ranging from self.minY-up, baseline->up, self.maxY->down by HBar.
        self._verticalCounters = None
        self._verticalRoundCounters = None
        self._verticalMixedCounters = None
        ...
        self._hints = None # DEPRECATED, use self.dimensions instead.
        self._dimensions = None # Store manual designer references to glyph dimensions (UFO only)

        '''
        Unfortunately we have to clear the glyphPath here, because the glyph
        outline may have changed, resulting in the reset request. Is there a
        way of testing if if really changed? And is that worth checking, as
        normally generating the glyph path is a very fast operation by Cocoa.
    ...
    def initialize(self):
        """Regardless of who gets called first, this will do the initializing
    ...
    def initializeFuzz(self):
        """The @initializeFuzz@ method answers the @FUZZ@ value: the amount
    ...
    def validate(self):
        """Runs several tests on the glyph and answers a list with report
        lines."""
        errors = []
        style = self.glyph.font

        for component in getComponents(self.glyph):
            if not component.baseGlyph in style:
                errors.append('Glyph "%s-->%s" component base "%s" missing.' % (TX.path2FileName(style.path), self.glyph.name, component.baseGlyph))

        return errors

    def _get_glyphPath(self):
        """Caches the path for fast point inside/outside query."""
    ...
    def isEqualShape(self, glyph):
        """Answers if glyph is the same as the glyph of self, ignoring the
        sidebearings and correcting the position of the bounding box on the
        left side bearing."""
    ...
    def __eq__(self, glyph):
        """Answers if glyph is the same as the glyph of self."""
        return self.isEqualShape(glyph)

    def getAnalyzer(self, name):
        """Answers the analyzer of the another glyph, through the parent,
    ...
    def _get_decomposedName(self):
        ...
        list of diacritics and a list of extensions, by splitting on glyph /
        accent name patterns. Exceptions to this method are ('Thorn' and
        'thorn').

        The returned dictionary has the following entries for Igrave.sc

        {'srcName': 'Igrave', 'diacritics': ['grave'], 'extensions': ['sc'],
        'name': 'Igrave.sc', 'coreName': 'I', 'baseName': 'I.sc',
        'altBaseName': 'I.sc'}
        ...
        if self._decomposedName is None:
            if self.name in ('Thorn', 'thorn') or self.name.startswith('.'):
                ...
                # Otherwise altBaseName is equal to baseName. And also ignore
                # if this is a smallcap.
    ...
    def getGlyph(self, glyphName):
        """Answers the glyph with this name or None if it does not
        exist."""
        style = self.style
    ...
    def hasContours(self):
        """Answers if the glyph has contours."""
        return len(self.contours) > 0

    def _get_contours(self):
        if self.glyph is not None:
            contours = []
            for contour in self.glyph._contours:
                contours.append(contour._points)
            return contours
        return []
    ...
    def _get_relevantHeight(self):
        """Answers the height of the group that fits best on the basename glyph
        of self."""
    ...
    def _get_boundingBox(self):
        """Answers the BoundingBox instance, made from Vertical and Horizontal
        ...
        Vertical / Horizontal / PointContext instances, they should not be
        passed between glyphs. Use the 4-tuple of self.boundings to get the
        (x1, y1, x2, y2) as expanded rect of all nested components."""
    ...
    def _get_boundings(self):
        """Answers the tuple bounding box for self, including the component
    ...
    def _get_vacuumeBox(self, minX=-10000000, minY=-10000000, maxX=10000000, maxY=10000000):
        """The @getVacuumeBox@ function answers the vacuumed box around all
        points that are located in the rectangle @(minX, minY, maxX, maxY)@.
        This way it is possible to get the vacuume box for all points, but also
        from a selection of points inside the given area, including the
        transformed component references. The tricky thing is what to do with
        letters like "V". We we'll also cut across the letter on x-height and
        takes these points into the comparison as well."""
        bMinX = maxX
        bMaxX = minX
        bMinY = maxY
        bMaxY = minY
    ...
    def getPointContext(self, index):
        """Answers the point context indicated by _index_. If the index is out
        of range, then answer `None`."""
    ...
    def _get_components(self):
        """The @self.components@ method find all components of the glyph. If
    ...
    def findComponents(self):
        """The @findComponents@ method answers a list of @Component@ instances
    ...
    def _get_averageStemSize(self):
        """Answers the average float value of all stems found in the glyph.
        """
        total = 0
        stems = self.stems
    ...
    def _get_straightRoundStems(self):
        """Stems that are round extreme on one side and straight on the other
        side."""
    ...
    def _get_straightRoundBars(self):
        """Bars that are round extreme on one side and straight on the other
        side."""
    ...
    def _get_minStem(self):
        """Answers the minimal stem of all stems found in the glyph.
        """
    ...
    def _get_maxStem(self):
        """Answers the maximal stem of all stems found in the glyph."""
        stems = self.stems.keys()
        if stems:
            return max(stems)
        return None
    ...
    def _get_minRoundStem(self):
        """Answers the minimal round stem of all round stems found in the glyph.
        """
    ...
    def _get_maxRoundStem(self):
        """Answers the maximal round stem of all round stems found in the glyph."""
        roundStems = self.roundStems.keys()
        if roundStems:
    ...
    def _get_terminals(self):
        """Terminals are recognizes stem and bar endings where the point contexts are connected."""
    ...
    def initializePointContexts(self):
        ...
        for contourIndex, contour in enumerate(self.contours):
            ...
            if len(contour) < 3: # Ignore single unclosed lines
                continue
            while len(contour) < 5:
            ...
            for index, point in enumerate(contour):
                ...
                prev4 = index - 4 # Automatic running over to -4 index
                next1 = index + 1
                ...
                if next3 >= len(contour):
                    next3 = 0
                next4 = next3 + 1
                if next4 >= len(contour):
                    next4 = 0

                try:
                    pc = self.POINTCONTEXTCLASS(contour[prev4], contour[prev3], contour[prev2], contour[prev1],
                                                point, contour[next1], contour[next2],
                                                contour[next3], contour[next4], index, pIndex, contourIndex, clockwise)
                except IndexError as e:
                    print('Error in %s' % self)
                    print(e)
    ...
    def findSideBearings(self):
        """Initializes the `self._boundingBox` and answer it."""
        bb = self.boundingBox
    ...
    def intersectLine(self, p1, p2, canHaveComponent=False):
        """The @intersectLine@ method answers the intersection points of the
    ...
    def intersect(self, pc0, pc1, nextP=False):
        """The @intersect@ method test if the line from @pc0.p@ to @pc1.p@ is
        ...
        _next_ is @True@ (default is @False@ then test on the next point of
        the point contexts."""
    ...
    def spanRoundsOnBlack(self, pc0, pc1):
        """Answers if the line between _pc0_ and _pc1_ just spans black
        area."""
    ...
    def coveredInBlack(self, x, y):
        """The fast glyphPath test always answers `True` if the point is
    ...
    def lineInBlack(self, pc0, pc1=None, step=SPANSTEP):
        """Answers if the line (including the points) is totally covered in
        black."""
        if pc1 is None:
            pc1 = pc0.p1
    ...
    def onWhite(self, x, y):
        """Answers if the single point `(x, y)` is on white."""
        return not self.onBlack(x, y)

    def coveredInWhite(self, x, y):
        """The fast glyphPath test always answers `True` if the point is exact
        on the contour.  So we'll test the "+" shape (not square) that is one
        unit around the point. All of the 5 points must be black to decide if
        the point `(x, y)` is covered in black. Note that this gives a
        difference result than `not self.onBlack(x, y)`."""
    ...
    def spanWhite(self, x1, y1, x2, y2, step=SPANSTEP):
        ...
        between _pc0_ and _pc1_ are all on white area of the glyph. If step is
    ...
    def isSameHorizontalRoundExtreme(self, pc0, pc1):
        """ The @isSameHorizontalRoundExtreme@ method answers @True@ if
        _pc0_ and _px2_ are both left or both right round extremes.
        """
    ...
    def isSameVerticalRoundExtreme(self, pc0, pc1):
        """The `isSameVerticalRoundExtreme` method answers @True@ if
        _pc0_ and _px2_ are both top or both bottom round extremes.
        """
    ...
    def matchStem(self, p0, p1):
        """Find the matching stem that is located on the x positions of p0 and
        p1. Answers `None` if not matching stem can be found."""
        for stems in self.stems.values():
            for stem in stems:
                x01 = stem.x
    ...
    def isStem(self, pc0, pc1):
        """The `isStem` method takes the point contexts _pc0_ and _pc1_ to
        compare if the can be defined as a “stem”: if they are not round
        extremes, if the two point contexts have overlap in the vertical
        directions (being part of the same window) that runs on black and if
        both lines are not entirely covered in black.
        """
        # Fix for now: self.isPortrait(pc0, pc1), may not work for extreme wide type.

        return not pc0.isHorizontalRoundExtreme()\
            and not pc1.isHorizontalRoundExtreme()\
            and pc0.isVertical() and pc1.isVertical()\
            and self.isPortrait(pc0, pc1)\
    ...
    def matchRoundStem(self, p0, p1):
        """Find the matching round stem that is located on the x positions of
        p0 and p1. Answers `None` if not matching stem can be found."""
        for roundStems in self.roundStems.values():
            for roundStem in roundStems:
                x01 = roundStem.x
                if p0.x in x01 and p1.x in x01: # Test for swapped values too, assume p0.x != p1.x
    ...
    def isRoundStem(self, pc0, pc1):
        """The `isRoundStem` method answers if the _pc0_ and _pc1_ define a
        round stem. This is @True@ if one of both of point contexts are
        extremes and if both, they must “bend” in the same direction.
    ...
    def isStraightRoundStem(self, pc0, pc1):
        """Answers if one of (pc0, pc1) is a straight extreme and the other is
        a curve extreme.  The stem is only valid if the y of the curve extreme
        is between the y of the 2 vertical on-curves."""
    ...
    def isHorizontalCounter(self, pc0, pc1):
        """Answers if the connection between pc0.x and pc1.x is running
    ...
    def matchBar(self, p0, p1):
        """Finds the matching bar that is located on the y positions of p0 and
        p1. Answers `None` if not matching stem can be found."""
        for bars in self.bars.values():
            for bar in bars:
                y01 = bar.y
    ...
    def isBar(self, pc0, pc1):
        """The `isBar` method takes the point contexts _pc0_ and _pc1_ to
        ...
        black."""
        # TODO @@@@@@@ Buggy for sans-H, also finds capHeight. Needs to be fixed.
        # Fix for now: self.isLandscape(pc0, pc1), may not work for extreme wide type.

        return not pc0.isVerticalRoundExtreme()\
            and not pc1.isVerticalRoundExtreme()\
            and pc0.isHorizontal() and pc1.isHorizontal()\
            and self.isLandscape(pc0, pc1)\
            and self.middleLineOnBlack(pc0, pc1)\
            and self.overlappingLinesInWindowOnBlack(pc0, pc1)
            #and (not self.pointCoveredInBlack(pc0) or not self.pointCoveredInBlack(pc1))

    def matchRoundBar(self, p0, p1):
        """Find the matching round bar that is located on the y positions of
        p0 and p1. Answers `None` if not matching stem can be found."""
        for roundBars in self.roundBars.values():
            for roundBar in roundBars:
                y01 = roundBar.y
   ...
   # Also see HStem and OStem
    def _get_HBar(self):
        """Answer the (cached) minimal value of the
        self.parent['H'].bars.keys(), or from ('E', 'F') if the 'H' is not
        available in the font. The value is cached in self, and cleared by
        self.reset()."""
        if self._HBar is None:
            if self.name in ('H', 'E', 'F'): # self is one of the base glyphs, just take own stem
                self._HBar = min(self.bars.keys())
            else:
                self._HBar = min(self.parent['H'].bars.keys()) # self.parent also using ('H', E', 'F')
        return self._HBar
    HBar = property(_get_HBar)

    def _get_OBar(self):
        """Answer the (cached) minimal value of self.parent['C'].stems.keys(),
        or of 'C' if the 'O' is not available in the font. The value is
        cached in self, and cleared by self.reset()."""
        if self._OBar is None:
            if self.name in ('O', 'C'):
                self._OBar = min(self.roundBars.keys())
            else:
                self._OBar = min(self.parent['O'].roundBars.keys()) # self.parent also using ('O', 'C')
        return self._OBar
    OBar = property(_get_OBar)
    ...
    def findStems(self):
        ...
        NOTE: An alternative approach could be to make a Fourier analysis of
        all stem distances of the font, and so find out which are likely to
        have a stem distance.
        ...
        self._horizontalCounters = horizontalCounters = {}
        self._horizontalRoundCounters = horizontalRoundCounters = {}
        self._horizontalMixedCounters = horizontalMixedCounters = {}
        self._allHorizontalCounters = allHorizontalCounters = {} # Space between all neighboring stems, running over white only.
                    ...
                    for pc0 in vertical1:
                        for pc1 in vertical2:
                            ...
                            if self.isStem(pc0, pc1):
                                ...
                                if pc0.isVerticalExtreme() and pc1.isVerticalExtreme():
                                    if not size in horizontalCounters:
                                        horizontalCounters[size] = []
                                    horizontalCounters[size].append(counter)
                                elif pc0.isVerticalRoundExtreme() and pc1.isVerticalRoundExtreme():
                                    if not size in horizontalRoundCounters:
                                        horizontalRoundCounters[size] = []
                                    horizontalRoundCounters[size].append(counter)
                                else:
                                    if not size in horizontalMixedCounters:
                                        horizontalMixedCounters[size] = []
                                    horizontalMixedCounters[size].append(counter)

                                if not size in allHorizontalCounters:
                                    allHorizontalCounters[size] = []
                                allHorizontalCounters[size].append(counter)
    ...
    def findHints(self):
        ...
        for hint in self.glyph.lib.get(self.C.LIB_FLOQMEMESLIST, []):
            ...
            # Make "fake" point contexts, where the next and prev point are the
            # two neighboring interpolation points of the locations. pc0 and
            # pc1 are then the intermediate points of the interpolation
    ...
    def _get_blueBars(self):
        ...
        if self._blueBars is None:
            minYPoint = self.minYPoint
            maxYPoint = self.maxYPoint
            bar = self.HBar or self.OBar
            if self.bars:
                bar = min(self.bars.keys())
            elif self.roundBars:
                bar = min(self.roundBars.keys())
            self._blueBars = {
                minYPoint.y: self.BLUEBARCLASS(minYPoint.y, bar, self.name),
                0: self.BLUEBARCLASS(0, bar, self.name),
                maxYPoint.y: self.BLUEBARCLASS(maxYPoint.y, -bar, self.name)
            }
        return self._blueBars
    blueBars = property(_get_blueBars)


    # Also see HBar and OBar
    def _get_HStem(self):
        """Answer the (cached) minimal value of self.parent['H'].stems.keys(),
        or of ('I', E', 'F') if the 'H' is not available in the font. The
        value is cached in self, and cleared by self.reset()."""
        if self._HStem is None:
            if self.name in ('H', 'I', 'E', 'F'): # self is one of the base glyphs, just take own stem
                self._HStem = min(self.stems.keys())
            else:
                self._HStem = min(self.parent['H'].stems.keys()) # self.parent also using ('H', 'I', E', 'F')
        return self._HStem
    HStem = property(_get_HStem)

    def _get_OStem(self):
        """Answer the (cached) minimal value of self.parent['C'].stems.keys(),
        or of 'C' if the 'O' is not available in the font. The value is cached
        in self, and cleared by self.reset()."""
        if self._OStem is None:
            if self.name in ('O', 'C'):
                self._OStem = min(self.roundStems.keys())
            else:
                self._OStem = min(self.parent['O'].roundStems.keys()) # self.parent also using ('O', 'C')
        return self._OStem
    OStem = property(_get_OStem)
    ...
    def findSerifs(self):
        ...
        - Start and end with a stem or bar
        - Start with a stem or bar and end with a (local) extreme
        - Start with a (local) extreme and end with a stem or bar
        """
    ...
    # self.stem
    def _get_stem(self):
        """Answers the left most stem as `Stem` instance."""
        stem = None
        stems = self.stems
        if stems:
    ...
    # self.horizontalCounters  Answers the straight horizontal counters.
    def _get_horizontalCounters(self):
        if self._horizontalCounters is None:
    ...
    # self.horizontalRoundCounters  Answers the round horizontal counters.
    def _get_horizontalRoundCounters(self):
        if self._horizontalRoundCounters is None:
            self.findStems() # Also finds all horizontal counters.
        return self._horizontalRoundCounters
    horizontalRoundCounters = property(_get_horizontalRoundCounters)

    # self.horizontalMixedCounters  Answers the straight/round and round/straight horizontal counters.
    def _get_horizontalMixedCounters(self):
        if self._horizontalMixedCounters is None:
            self.findStems() # Also finds all horizontal counters.
        return self._horizontalMixedCounters
    horizontalMixedCounters = property(_get_horizontalMixedCounters)
    ...
    # self.verticalCounters  Answers the straight vertical counters.
    def _get_verticalCounters(self):
        if self._verticalCounters is None:
            self.findBars() # Also finds all vertical counters.
        return self._verticalCounters
    verticalCounters = property(_get_verticalCounters)

    # self.verticalRoundCounters  Answers the round vertical counters.
    def _get_verticalRoundCounters(self):
        if self._verticalRoundCounters is None:
            self.findBars() # Also finds all vertical counters.
        return self._verticalRoundCounters
    verticalRoundCounters = property(_get_verticalRoundCounters)

    # self.verticalMixedCounters  Answers the straight/round and round/straight vertical counters.
    def _get_verticalMixedCounters(self):
        if self._verticalMixedCounters is None:
            self.findBars() # Also finds all vertical counters.
        return self._verticalMixedCounters
    verticalMixedCounters = property(_get_verticalMixedCounters)
    ...
    def _get_roundStem(self):
        """Use the left round stem, if there are multiple in the glyph."""
        rstem = None
    ...
    def _get_bar(self):
        """Answers the left most bar as `Stem` instance."""
        bar = None
        bars = self.bars
        if bars:
    ...
    # self.allBars      Answers the combination dict of bars, blueBars and round bars
    ...
    def _get_allBars(self):
        if self._allBars is None:
            ...
            # For now keep blueBars out of list, needs more Dimensioneer testing.
            #for value, blueBar in self.blueBars.items():
            #    if not value in self._allBars:
            #        self._allBars[value] = []
            #    self._allBars[value].append(blueBar)
    ...
    def _get_allBarsCountersComponents(self):
        if self._allBarsCountersComponents is None:
            ...
            for value, bars in self.allBarsCounters.items():
                # Make copy, as we'll appending to it.
                self._allBarsCountersComponents[value] = bars[:]

            # Now add bars from the component glyphs. We don't do the counters
            # of the components.
            for component in getComponents(self.glyph):
                if component.baseGlyph in self.style:
                    # We may need to transform the found stems.
                    offset = component.transformation[-2:]
                    cga = self.parent[component.baseGlyph]
                    for size, bars in cga.allBars.items():
                        if not size in self._allBars:
                            self._allBars[size] = []
                        for bar in bars:
                            # Add offset to copy. Use as (y1, y2) = bar.y to
                            # calc position + offset.
                            bar = bar.copy()
                            # Incremental if multiple chained component
                            # reference.
                            bar.addOffset(offset)
    ...
    def _get_minBar(self):
        """Answers the minimal bar of all bars found in the glyph."""
        bars = self.bars.keys()
        if bars:
            return min(bars)
        return None
    ...
    def _get_maxBar(self):
        """Answers the maximal bar of all bars found in the glyph."""
        bars = self.bars.keys()
        if bars:
            return max(bars)
        return None
    ...
    def _get_roundBar(self):
        """Use the left round bar, if there are multiple in the glyph."""
        rbar = None
    ...
    def _get_minRoundBar(self):
        """Answers the minimum round bar of all round bars found in the
        glyph."""
    ...
    def _get_maxRoundBar(self):
        """Answers the maximum round bar of all round bars found in the
        glyph."""
        roundBars = self.roundBars.keys()
        if roundBars:
    ...
    def _get_rightMargin(self):
        """Answers the rightMargin value. Use `self.boundingBox.maxX` to
        get the `Vertical` instance that defines the right margin. This
        way the point contexts involved can be queried. If there are no
        outlines (`self.boundingBox is None`, then answer 0 by definition
    ...
    def _get_leftMargin(self):
        """Answers the leftMargin value. Use `self.boundingBox.minX` to
        get the `Vertical` instance that defines the left margin. This way
        the point contexts involved can be queried. If there are no outlines
        (`self.boundingBox is None`, then answer 0 by definition
        (compatible with RoboFont)). Answers default 0 if glyph does not exist
        in the style."""
    ...
    def findNearestBlack(self, x, y, radius=2):
        """Make a horizontal beam, and measure the distances to the nearest
        black, if (x, y) is not black from itself."""
        glyph = self.glyph
    ...
    def _get_leftBaseMargin(self):
        """Answers the left angled margin of the base glyph (referred to by
        ...
        sure."""
        # In altBaseName, "i" and "j" are translated in "dotlessi" and
        # "dotlessj"
        baseName = self.decomposedName['altBaseName']

        for component in getComponents(self.glyph):
            # Answers transformed left margin of this glyph.
            if component.baseGlyph == baseName:
                if baseName in self.parent:
        ...
        # No referencing components, just answer its own angledLeftMargin
        return self.angledLeftMargin

    """
    def _set_leftBaseMargin(self, margin):
        # ONLY READLINE for now. Not clear how to solve some consistency
        # problems with other glyphs referring to self.glyph
        marginDiff = 0
        baseName = self.decomposedName['altBaseName']
        for component in getComponents(self.glyph):
            if component.baseGlyph == baseName: # Answers transformed left margin of this glyph
                if not baseName in self.parent:
                    break
                leftMargin = self.parent[baseName].angledLeftMargin
                if leftMargin is not None:
                    marginDiff = margin - leftMargin - component.transformation[4]
                else:
                    marginDiff = margin - leftMargin
                break
        self.angledLeftMargin += marginDiff
    """
    leftBaseMargin = property(_get_leftBaseMargin)

    def _get_rightBaseMargin(self):
        """Answers the right angled margin of the base glyph (referred to by
        the component that has the same base name as self. If there is a
        "dotless"+self.name in the style, then take that as base for accent
        ...
        # No referencing components, just answer its own angledRightMargin
    ...
    rightBaseMargin = property(_get_rightBaseMargin)

    # self.width

    def _get_width(self):
        """Answers the glyph width. This method is for compatibility reason,
        there is no `PointContext` involved."""
        glyph = self.glyph
        if glyph is None:
            return 0 # Default value if glyph does not exist in the style.
        return glyph.width
    ...
    def _get_unitsPerEm(self):
        """Answers the units per em for this font. Identical to
        `self.style.info.unitsPerEm`."""
    ...
    def guessAnchorPosition(self, position):
        """Answers the estimated position (x, y) of the anchor for the glyph of
        this analyzer. There are several options, depending on the content of
        the position attribute. If the position is close to xheight or
        baseline, then answer that y position of the glyph. Otherwise the
        position is answered relative to what x-height would have been. This
        way the accents are assumed to be positioned on x-height and the other
        vertical positions are moving relative to that.

        The _position_ attriibute is one of `(C.ANCHORTOP, C.ANCHOR_TOP,
        C.ANCHORBOTTOM, C.ANCHOR_BOTTOM, C.ANCHORCENTER, C.ANCHOR_CENTER)`.
        ...
        if position == self.C.ANCHORTOP:
            # Get the x-position on top of the glyph, averaged by the y
            # position of the point.  The more vertical distance there is,
            # relative to the top point, the less this x position will add to
            # the total.
            maxY = self.maxY
            if maxY is None:
                x = y = 0
    ...
    def getAverageXByWeightedScanlineYDistance(self, py):
        """Answers the averaged x value by measuring against black parts of a
        set of scan lines. This way we can calculate the average middle point
        of all scan lines, but weighted against the distance to the target py
    ...
    def _getScanLineSumCount(self, py, edges):
        """Better not to use a for loop, since the step is changing."""
        xList = []
    ...
    def getLeftWhiteScanLines(self, step):
        """Answers the list of left white scan lines from _py1_ to _py2_ by
        _step_."""
        lines = []
    ...
    def getPointByNamedValue(self, name):
        """Answers the value (as point, where x or y depend on the type of
        value) as indicated by name."""
