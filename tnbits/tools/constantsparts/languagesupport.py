...
class LanguageSupport(object):
    ...
    SPECIAL_CHARS_BY_LANGUAGE_REQUIRED: Mostly sourced from <http://www.eki.ee/letter/>. These are required special characters for the basic typesetting of each language. It is assumed that Basic Latin is required to write all of them, even though. Caveats simplified or commented out where possible.
    SPECIAL_CHARS_BY_LANGUAGE_IMPORTANT: Mostly sourced from <http://www.eki.ee/letter/>. These are letters which are important to the language, but not required in everday setting. Mostly due to common loanwords or regional names/placenames.
    SPECIAL_SORTS_BY_LANGUAGE: Not done yet. Thinking about special currency glyphs or quotation glyphs that are used. <http://en.wikipedia.org/wiki/Non-English_usage_of_quotation_marks>

    Issues:
        - Special characters for Cyrillic is missing.

        - OpenType language tags do not cover all of the language listings. We may want to switch to (openTypeScript, ISOcode) tuples.

        - Different sources have different results. Need to some how get everything on the same page.
    ...
    # Belarusian
    'BEL': [262, 263, 268, 269, 321, 322, 323, 324, 346, 347, 352, 353, 364, 365, 377, 378, 381, 382],
    # Bislama
    ...
    # Catalan
    'CAT': [192, 224, 199, 231, 200, 232, 201, 233, 205, 237, 207, 239, 319, 320, 210, 242, 211, 243, 218, 250, 220, 252],
    #Czech
    'CSY': [193, 225, 268, 269, 270, 271, 201, 233, 282, 283, 205, 237, 327, 328, 211, 243, 344, 345, 352, 353, 356, 357, 218, 250, 366, 367, 221, 253, 381, 382],
    #Welsh
    'WEL': [193, 225, 192, 224, 194, 226, 196, 228, 201, 233, 200, 232, 202, 234, 203, 235, 205, 237, 204, 236, 206, 238, 207, 239, 211, 243, 210, 242, 212, 244, 214, 246, 218, 250, 217, 249, 219, 251, 220, 252, 221, 253, 7922, 7923, 374, 375, 376, 255, 372, 373],
    #Danish
    ...
    #Vietnamese
    'VIT': [192, 224, 193, 225, 194, 226, 7846, 7847, 7844, 7845, 7850, 7851, 7848, 7849, 7852, 7853, 195, 227, 258, 259, 7856, 7857, 7854, 7855, 7860, 7861, 7858, 7859, 7862, 7863, 7842, 7843, 7840, 7841, 272, 273, 200, 232, 201, 233, 202, 234, 7872, 7873, 7870, 7871, 7876, 7877, 7874, 7875, 7878, 7879, 7868, 7869, 7866, 7867, 7864, 7865, 204, 236, 205, 237, 296, 297, 7880, 7881, 7882, 7883, 210, 242, 211, 243, 212, 244, 7890, 7891, 7888, 7889, 7894, 7895, 7892, 7893, 7896, 7897, 213, 245, 7886, 7887, 416, 417, 7900, 7901, 7898, 7899, 7904, 7905, 7902, 7903, 7906, 7907, 7884, 7885, 217, 249, 218, 250, 360, 361, 7910, 7911, 431, 432, 7914, 7915, 7912, 7913, 7918, 7919, 7916, 7917, 7920, 7921, 7908, 7909, 7922, 7923, 221, 253, 7928, 7929, 7926, 7927, 7924, 7925],
    #Wolof
    'WLF': [192, 224, 195, 227, 201, 233, 203, 235, 209, 241, 330, 331, 211, 243],
    #Walloon
    'WLN': [197, 229, 194, 226, 199, 231, 202, 234, 201, 233, 200, 232, 206, 238, 212, 244, 219, 251],
    #Yapese
    'YAP': [196, 228, 203, 235, 214, 246],
    #Yoruba
    'YBA': [192, 224, 193, 225, 194, 226, 461, 462, 200, 232, 201, 233, 202, 234, 282, 283, 7864, 7865, 204, 236, 205, 237, 206, 238, 463, 464, 57344, 57345, 323, 324, 57356, 57357, 210, 242, 211, 243, 212, 244, 465, 466, 7884, 7885, 7778, 7779, 217, 249, 218, 250, 219, 251, 467, 468],
    #####################################################
    ...
    SUPPORT_NTS = ['AFK', 'SQI', 'EUQ', 'BRE', 'BOS', 'CAT', 'HRV', 'CSY', 'DAN', 'NLD', 'ENG', 'NTO', 'ETI', 'FOS', 'FJI', 'FIN', 'FLE', 'FRA', 'FRI', 'DEU', 'GRN', 'HAW', 'HUN', 'ISL', 'IND', 'IRI', 'ITA', 'LAT', 'LVI', 'LTH', 'MLY', 'MTS', 'MRI', 'MOL', 'NOR', 'PLK', 'PTG', 'PRO', 'RMS', 'ROM', 'ROY', 'ISM', 'LSM', 'NSM', 'SSM', 'SMO', 'GAE', 'SKY', 'SLV', 'SRB', 'ESP', 'SWK', 'SVE', 'PIL', 'TRK', 'WEL'],


    # This lists language support by codepage. THESE ARE NOT WELL-SOURCED LISTS, and may conflict with language information given elsewhere.

    SUPPORT_BY_CODEPAGE = {
                           # Latin-1 Basic Latin
                           'ISO_8859_1': ['AFK', 'SQI', 'BRE', 'CAT', 'DAN', 'ENG', 'FOS', 'GAL', 'DEU', 'ISL', 'IRI', 'ITA', 'KUR', 'LAT', 'LTZ', 'NOR', 'OCI', 'PTO', 'RMS', 'GAE', 'ESP', 'SWK', 'SVE', 'EUQ', 'Waloon', 'Leonese'] +
['NLD', 'FLE', 'ESI', 'FRA', 'FIN'],
...
#http://www.eki.ee/letter/
    ...
    I and U with acute were moved from required to important as obsolete. Current use favours grave instead of acute.
    I with circumflex is added as important.
    The circumflex is used on an -i ending that was anciently written -ii (or -ji, -ij, -j, etc.) to distinguish homograph plurals and verb forms; e.g. princip (p. of principio = principle) from principi (p. of principe = price), gen (p. of genio = genius) from geni (p. of gene = gene), etc.
    A with acute is added as important.
    While it does not occur in ordinary running texts, geographical names on maps are often written only with acute accents.


    KUR
    No common alphabet has yet been agreed on.
    The required characters section above is taken from the unified Kurdish alphabet as proposed by Kurdish Academy of Language. This alphabet additionally has JH, LL, RR and SH as separate characters and fits into ISO 8859-1 character set.
    Other Latin alphabets (apart from Cyrillic and Arabic) exist for Kurdish of which Kurmanji (containing E, I and U with circumflex, C and S with cedilla) is perceived as the most widespread. Kurmanjî characters are listed as important.
    The repertoire proposed by Kurdish Worldwide Resources site also includes letter AE.
    ...
    The government of the Latvian SSR decreed that the letters R WITH CEDILLA and O WITH MACRON, along with the LIGATURE CH be removed from written Latvian. Since then, there have existed two different Latvian writing systems. The Latvian community outside of Latvia continues to use the pre-occupation system, while in Latvia the changes made by the Soviet regime are still in use today. There really has been no major move to reform Latvian or to completely adopt either system. So, for the moment, these differences are likely to remain.

    NLD
    > One official unicode source says Dutch needs:
    The acute accent is used as a stress marker in Dutch. It's pretty much equivalent to writing a word in bold or italics. The accent can be placed on any vowel or vowel combination. A typical example is the numeral "een" (one), which is often written as "n" to distinguish it from the article "een" (a, an).

    >     (but not ; can you give an example using ?)

    A rare example is the biblical name "Kanan", which is pronounced with three syllables. Without the dieresis "naan" would stand for one syllable. We also used to write "napen" ('na' after + 'apen' = to ape, to mimick), but since the 1995 spelling reform, the dieresis is replaced by a hyphen: "na-apen".

    Letter Database comments:  is used in vacum.

    >

    Accents occur in French loanwords only as in , caf, crme, enqute, reu, matre. Those loanwords that have been fully adopted into the Dutch language lose accents on all vowels except 'e'. Hence "caf", "controle", "crpe", "pat", "ragout", "scne", "volire".

    The letter combination "ij" is a Dutch diphthong that sounds like "eille" in the French name "Mireille". In certain word endings "ij" is just a schwa. You could consider it a letter in its own right, because the uppercase version is not "Ij" but "IJ", e.g. "IJsland" (Iceland). But then again, all authoritative dictionaries alphabetize "ij" as simply i + j.

    Since "ij" is a vowel (a diphthong, to be precise), it can also carry a stress marker. If technically possible, an acute accent should be written over i as well as j, so the spelling rule says. But usually that isn't possible at all, so we write "j" instead.

    There's no such thing as a Flemish language. The northern half of Belgium is called Flanders and the people who live there are the Flemish. But their mother-tongue is 'Nederlands' (Dutch, Niederlndisch, nerlandais, etc.). Apart from a recognizable southern accent and some typical regionalisms in the vocabulary, it's the same language as the one spoken in the Netherlands. Spelling rules are identical in both countries and, as a rule, major Dutch dictionaries and grammar books are compiled and edited by a mixed staff of Dutch and Flemish linguists. Contrary to what many people believe, the Dutch language in Belgium is officially NOT called 'Flemish'.
    ...
    grave accent is used when there is a contraction of preposition a with an article or pronoun: a + a = à (à casa, to the house).
    EGRAVE and OGRAVE are used in geographical names outside Europe and not part of the language proper. The now abandoned practice was to indicate underlying stress in words ending in -mente -- se`mente, u`ltimamente etc.
    K, W and Y are used only in foreign words.

    The diaeresis on U is now used only in Brazil.

    Feminine and masculine ordinal indicators (U+00AA and U+00BA) are used in Portuguese.



    SLV
    The 'important' characters are not part of the alphabet. Dictionaries additionally use ortographic signs for stress and pronunciation:
    ...
    A WITH GRAVE is added to the original data. After all, AltaVista suggests that there exist more than 10 million pages in Swedish containing this character.
    Saami place-names use special orthography on older maps, conversion to the new Saami orthography is in progress. Swedish Saami Council has proposed that Ume Saami orthography be used additionally, adding D WITH STROKE and T WITH STROKE to the character repertoires of South, Lule and North Saami.
    ...
    YBA
    Although the "dot below" diacritic is widely used, purists prefer a short vertical underbar (Unicode COMBINING VERTICAL LINE BELOW U+0329) - this resembles the IPA notation for a syllabic consonant, attached to the base of the letter (E, O or S).
    The seven Yoruba vowels (A, E, E underbar, I, O, O underbar, U) can be uttered in three different tones: high (acute accent); middle (no accent) and low (grave accent).
    The letters M and N, when written without diacritics, indicate nasalisation of the preceding vowel.
    M and N also occur as syllabics - in these circumstances, they take acute or grave tonal diacritics, like the vowels. Middle tone is also marked (with a macron) to differentiate it from the unmarked nasalising consonants.
    A tilde was used in older orthography to indicate a double vowel. This was tonally ambiguous, and has now been replaced by showing the paired vowels, each marked with the appropriate tones.
    However, where a double vowel has the tonal sequence high-low or low-high, it may optionally be replaced by a single vowel with a circumflex (high-low) or caron (low-high), eg. A acute + A grave = A circumflex; A grave + A acute = A caron.
    Tilde characters are "important" rather than "required", since they are only used to render the old orthography.
