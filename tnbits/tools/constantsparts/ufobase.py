...
class UfoBase:
    ...
    # For a complete set of fields see /XierpaDev/Applications/TypeNetworkLibrary/src/typenetworklibrary/_py/models.py.
    # Fields used as attributes in floqs must have camelCase to be compatible with standard fields in RoboFont
    ...
    # Some queries don't need the full item set as default, so the query/url can defined the fields.
    # FLOQS_ITEMBASE list below is always included.
    FLOQS_ITEMBASE = [
    ...
    # The full read includes all search words. These are normally only used for writing
    # as they are for searching in the database.
