...
#
#    Path of working
#    • Check spacing and margins for all diacritics and tab glyphs. Mark them in glyph.lib a locked spacing.
...
class KernNetSpacingAssistant(Subscriber):
    ...
    def calibrate(self, g, margin=500):
        ...
        for m in range(500): # Number of iterations, avoid infinite loop
            ...
            k1, k2 = self.predictKerning(g.name, g, g.name, step=1)
    ...
    def spaceButtonCallback(self, sender):
        ...
        if g is not None:
            if self.w.spaceAll.get():
                ...
                for ggName in sorted(g.font.keys()):
                    gg = g.font[ggName]
                    if gg.components: # Then do all glyphs with components.
                        print(f'... Calibrating margins of component glyph /{gName}')
                        assistant.space(gg)
            else:
                assistant.space(g)
