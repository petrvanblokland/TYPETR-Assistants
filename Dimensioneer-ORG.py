...
class Dimensioneer(BaseTool):
    ...
    def setPreferences(self):
        ...
        r, g, b, t = self.getPreference('gridColor')
        self._drawComponentDimensions = self.getPreference('drawComponentDimensions')
        self._gridColor = NSColor.colorWithCalibratedRed_green_blue_alpha_(r, g, b, t)
        r, g, b, t = self.getPreference('stemColor')
        ...
        self._stemColor = NSColor.colorWithCalibratedRed_green_blue_alpha_(r, g, b, t)
        r, g, b, t = self.getPreference('metricsColor')
        self._metricsColor = NSColor.colorWithCalibratedRed_green_blue_alpha_(r, g, b, t)
        r, g, b, t = self.getPreference('counterColor')
        self._counterColor = NSColor.colorWithCalibratedRed_green_blue_alpha_(r, g, b, t)
    ...
    @classmethod
    def open(cls):
        """Open Dimensioneer only once."""
        if cls.exists():
            tool = cls.getToolById(cls.TOOLID)
            tool.terminate()
    ...
    def drawDiagonals(self):
        ...
        for distance, pp0, pp1 in self._dValues:
            ...
            if distance > 0:
                color.set()
                path = NSBezierPath.bezierPath()
                path.moveToPoint_((pp0.x, pp0.y))
                path.lineToPoint_((pp1.x, pp1.y))
                path.setLineWidth_(0.4 * self.scale)
                path.stroke()
...
def getDimensioneer():
    Dimensioneer.open()
    return Dimensioneer.getTool()

if __name__ == '__main__':
    tool = getDimensioneer()
