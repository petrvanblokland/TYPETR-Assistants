...
class MerzElement:
    ...
    def newGlyphText(self, glyphNameLines, font, x=0, y=0, h=1, leading=None, 
        fill=None, name=None, parent=None, showMetrics=True, showBox=True):
        if parent is None:
            parent = self
        e = MerzGlyphText(glyphNameLines, font, x=x, y=y, h=h, leading=leading, fill=fill, name=name, parent=parent, 
            showMetrics=showMetrics, showBox=showBox)
        parent.addElement(e)        
        return e
...
class MerzGlyphLine(MerzElement):
    ...
    def __init__(self, glyphNames, font, x=0, y=0, h=1, fill=None, strokeWidth=1, stroke=None, 
        ...
        # Unpack kerning from groups for this line. Note that the font per line can be different,
        # so they keep their own x-ref glyph2Group tables.
        # Optimiszation can be to keep them on a more central place, so lines with identical fonts
...
class MerzGlyphText(MerzElement):
    """
    Element that shows a list of MerzGlyphLine instances with a certain leading.
    Special optimisation is that the lines cycle (only updating the veritcal position of the elements)
    when scrolling through a large text.    
    """
    def __init__(self, glyphNameLines, font, x=0, y=0, h=1, leading=None, fill=None, strokeWidth=1, stroke=None, 
        name=None, parent=None, showBox=True, showMetrics=True):
        MerzElement.__init__(self, name, parent)
        self.glyphNameLines = glyphNameLines # List of list of glyphnames, per line
        self.font = font
        self._x, self._y, self._h = x, y, h
        self._fill, self._strokeWidth, self._stroke = fill or WHITE, strokeWidth, stroke or None
        self.showBox = showBox
        ...
        self.glyphNameLines = glyphNameLines
        numLines = len(glyphNameLines) # Number of lines in the text
        if leading is None:
            leading = self.gh / numLines # @@@ Some better calculate need here to make them fit in self.h
        fontSize = leading # @@@ Better calculation needed here too
        
        # Rectangle to hold the line elements in a relative vertical position
        self.merzLayer = self.parent.merzLayer.appendRectangleSublayer(
            name=self.name,
            position=(self.gx, self.gy), # Calculate global position and size by properties
            size=(self.gw, self.gh),
            fillColor=self._fill,
            strokeWidth=self._strokeWidth,
            strokeColor=self._stroke
        ...
        self.glyphLines = [] # List of MerzGlyphLine instances, in top-to-bottom order
        
        y = self.gh
        x = 0

        for glyphNames in glyphNameLines:
            # Add the filling glyph path, that works inside a scaled full-em coordinate system 
            self.glyphLines.append(self.newGlyphLine(glyphNames, font=font, x=x, y=y, h=leading))
            y -= leading
    ...
    def updatePosSize(self):      
        ...
        for me in self.elements:
            me.updatePosSize()
        leading = self.gh / len(self.glyphLines) # @@@ Some better calculate need here to make them fit in self.h
        fontSize = leading # @@@ Better calculation needed here too
        y = self.gh
        x = 0
        for glyphLine in self.glyphLines:
            glyphLine.y = y
            glyphLine.h = leading
            y -= leading
...
class BaseAssistant(Subscriber):
    ...
    def keyDown(self, event):
        ...
        #    return
        if characters in self.FUNCTION_KEYS:
            self.FUNCTION_KEYS[characters](self, characters)
            """
            if characters == 'y':
                self.w.showSidesBlack.set(not self.w.showSidesBlack.get()) # Toggle the value
                self.update()
    ...
    def build(self):
        ...
        self.mr.newGlyphText((
            ...
            ('E', '/?', 'V', 'H', '/?', 'H', 'O', '/?', 'O', 'n', '/?', 'n'),
            ('F', '/?', 'V', 'H', '/?', 'H', 'O', '/?', 'O', 'n', '/?', 'n'),
            ), glyph.font, x=M, y=M, h=-M, fill=RED)
