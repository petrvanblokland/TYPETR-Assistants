...
ARROW_KEYS = [NSUpArrowFunctionKey, NSDownArrowFunctionKey,
        NSLeftArrowFunctionKey, NSRightArrowFunctionKey, NSPageUpFunctionKey,
        NSPageDownFunctionKey, NSHomeFunctionKey, NSEndFunctionKey]
FUNCTION_KEYS = ('=', '+', 'y', 'n', 'N', 'm', 'M', 'u', 'i', 'o', 'p', 'U', 'I', 'O', 'P', '.', ',', '<', '>')
...
class MerzElement:
    ...
    def newGlyph(self, glyph, x=0, y=0, h=1, fill=None, name=None, parent=None, showMetrics=True, showMargins=True):
        if parent is None:
            parent = self
        e = MerzGlyph(glyph, x=x, y=y, h=h, fill=fill, name=name, parent=parent, 
            showMetrics=showMetrics, showMargins=showMargins)
        parent.addElement(e)        
        return e

    def newGlyphLine(self, glyphNames, font, x=0, y=0, h=1, fill=None, name=None, parent=None, showMetrics=True, showMargins=True):
        if parent is None:
            parent = self
...
class MerzGlyph(MerzElement):
    def __init__(self, glyph, x=0, y=0, h=1, fill=None, strokeWidth=1, stroke=None, 
        name=None, parent=None, showMargins=True, showMetrics=True):
        MerzElement.__init__(self, name, parent)
        self.glyph = glyph
        ...
        #self.newLine(x=0, y=baseY+fontInfo.ascender, stroke=BLUE)   
    ...
    def setGlyph(self, glyph):
        ...
        for gIndex, glyphName in enumerate(self.glyphNames):
            ...
            if prevName is None:
                ...
                self.kerningLabels[gIndex].x = x + k
                self.kerningLabels[gIndex].setText('%d' % k)
                if k < 0:
                    c = RED
                elif k > 0:
                    c = GREEN
    ...
    def keyDown(self, event):
        ...
        if characters in FUNCTION_KEYS:
            if characters == 'y':
                ...
                self.update()
            elif characters == 'N': # Decrement left kerming by 20
