...
from assistantLib.assistantParts.glyphsets.Latin_L_set import LATIN_L_SET, LATIN_L_SET_ITALIC

LATIN_XL_SET_NAME = 'Latin XL'
LATIN_XL_SET_NAME_ITALIC = 'Latin XL Italic'

# The "c" attribtes are redundant, if the @uni or @hex are defined, but they offer easy searching in the source by char.

LATIN_XL_SET = deepcopy(LATIN_L_SET)
LATIN_XL_SET_ITALIC = deepcopy(LATIN_L_SET_ITALIC)
...
for GDS in (LATIN_XL_SET, LATIN_XL_SET_ITALIC):
...
if __name__ == '__main__':
    for gName, gd in LATIN_XL_SET.items():
        #print('---', gd)
        if gd.base and gd.base not in LATIN_XL_SET:
            print('##### Missing base', gName, gd.base)
        for aName in gd.accents:
            if aName not in LATIN_XL_SET:
