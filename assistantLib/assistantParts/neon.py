...
class AssistantPartNeon(BaseAssistantPart):
    ...
    def calculate(self, g, preserveComponents=True):
        ...
        drawOriginal = c.w.addOriginal.get()
        drawInner = c.w.addInner.get()
        drawOuter = c.w.addOuter.get()
    ...
    def buildNeon(self, y):
        ...
        thicknessValue = self.getLib(f, "TYPETR.%s.%s" %(outlinePaletteDefaultKey, "contrastAngle"), self.DEFAULT_THICKNESS)
        ...
        contrastValue = self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "contrast"), 0)
        ...
        contrastAngleValue = self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "contrastAngle"), 0)
        ...
        connectmiterLimitValue = self.getLib(f, "TYPETR.%s.%s" %(outlinePaletteDefaultKey, "connectmiterLimit"), True)

        self.w.connectmiterLimit = CheckBox((middle-22, y-3, 20, 17), "",
                                             callback=self.connectmiterLimit,
                                             value=connectmiterLimitValue)

        miterLimitValue = self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "miterLimit"), 10)
        ...
        useCapValue = self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "closeOpenPath"), True)
        ...
        cornerValue = self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "corner"), "Round")
        if cornerValue in cornerAndCap:
            self.w.corner.set(cornerAndCap.index(cornerValue))

        capValue = self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "cap"), "Round")
        ...
        self.w.keepBounds = CheckBox((middle-3, y, middle, 22), "Keep Bounds",
                                   value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "keepBounds"), False),
                                   callback=self.parametersTextChanged)
        y += 30
        self.w.addOriginal = CheckBox((middle-3, y, middle, 22), "Add Source",
                                   value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "addOriginal"), False),
                                   callback=self.parametersTextChanged)
        y += 30
        self.w.addInner = CheckBox((middle-3, y, middle, 22), "Add Left",
                                   value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "addLeft"), True),
                                   callback=self.parametersTextChanged)
        y += 30
        self.w.addOuter = CheckBox((middle-3, y, middle, 22), "Add Right",
                                   value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "addRight"), True),
                                   callback=self.parametersTextChanged)
        ...
        self.w.preview = CheckBox((middle-3, y, middle, 22), "Preview",
                               value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "preview"), False),
                               callback=self.previewCallback)
        y += 30
        self.w.fill = CheckBox((middle-3+10, y, middle, 22), "Fill",
                               value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "fill"), False),
                               callback=self.fillCallback, sizeStyle="small")
        y += 25
        self.w.stroke = CheckBox((middle-3+10, y, middle, 22), "Stroke",
                               value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "stroke"), True),
        ...
        self.w.color = ColorWell(((middle-5)*1.7, y-33, -10, 60),
                                 color=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "color"), color),
                                 callback=self.colorCallback)

        self.w.preserveComponents = CheckBox((10, -25, -10, 22), "Preserve Components", sizeStyle="small",
                                value=self.getLib(f, "%s.%s" %(outlinePaletteDefaultKey, "preserveComponents"), True),
                                callback=self.parametersTextChanged)
    ...
    def parametersChanged(self, sender=None, g=None):
        ...
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "thickness"), thickness)
        contrast = int(c.w.contrast.get())
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "contrast"), contrast)
        contrastAngle = int(c.w.contrastAngle.get())
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "contrastAngle"), contrastAngle)
        keepBounds = c.w.keepBounds.get()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "keepBounds"), keepBounds)
        preserveComponents = bool(c.w.preserveComponents.get())
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "preserveComponents"), preserveComponents)

        miterLimit = int(c.w.miterLimit.get())
        if c.w.connectmiterLimit.get():
            miterLimit = thickness
            c.w.miterLimit.set(miterLimit)
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "miterLimit"), miterLimit)

        corner = c.w.corner.getItems()[c.w.corner.get()]
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "corner"), corner )

        cap = c.w.cap.getItems()[c.w.cap.get()]
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "cap"), cap )

        drawOriginal = c.w.addOriginal.get()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "addOriginal"), drawOriginal)

        drawInner = c.w.addInner.get()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "addLeft"), drawInner)

        drawOuter = c.w.addOuter.get()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "addRight"), drawOuter)
    ...
    def connectmiterLimit(self, sender):
        c = self.getController()
        f = CurrentFont()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "connectmiterLimit"), sender.get())
    ...
    def useCapCallback(self, sender):
        ...
        f = CurrentFont()
        value = sender.get()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "closeOpenPath"), value)
    ...
    def previewCallback(self, sender):
        ...
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "preview"), value)
        self.updateView()

    def fillCallback(self, sender):
        f = CurrentFont()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "fill"), sender.get()),
        self.updateView()

    def strokeCallback(self, sender):
        f = CurrentFont()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "stroke"), sender.get()),
        self.updateView()

    def colorCallback(self, sender):
        f = CurrentFont()
        self.setLib(f, "%s.%s" %(outlinePaletteDefaultKey, "color"), sender.get())
        self.updateView()
